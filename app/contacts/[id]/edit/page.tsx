...
import { ChevronLeft, Camera, X, Plus, Trash2, Archive } from "lucide-react";
...
import type { ContactOffer, ContactNeed, ContactTag } from "@/types";
...
export default function EditContactPage({
  ...
  useEffect(() => {
    params.then(({ id }) => {
      ...
      Promise.all([
      ...
      ]).then(([{ data: c }, { data: o }, { data: n }, { data: t }]) => {
        if (!c) { router.push("/contacts"); return; }
        setName(c.name);
        setCategory(c.category ?? "");
        setHowWeMet(c.how_we_met ?? "");
        setNotes(c.notes ?? "");
        setEmail(c.email ?? "");
        setPhone(c.phone ?? "");
        ...
        setOffers((o ?? []).map((x: ContactOffer) => x.text));
        setNeeds((n ?? []).map((x: ContactNeed) => x.text));
        setTags((t ?? []).map((x: ContactTag) => x.tag));
        setLoading(false);
      });
    });
  }, [params, router]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (photoFile) {
      ...
      if (uploadError) {
        ...
        const { data: urlData } = supabase.storage.from("contact-photos").getPublicUrl(data.path);
    ...
    await Promise.all([
      supabase.from("contact_offers").delete().eq("contact_id", id),
      supabase.from("contact_needs").delete().eq("contact_id", id),
      supabase.from("contact_tags").delete().eq("contact_id", id),
    ]);

    await Promise.all([
      offers.length > 0
        ? supabase.from("contact_offers").insert(offers.map((text) => ({ contact_id: id, text })))
        : null,
      needs.length > 0
        ? supabase.from("contact_needs").insert(needs.map((text) => ({ contact_id: id, text })))
        : null,
      tags.length > 0
        ? supabase.from("contact_tags").insert(tags.map((tag) => ({ contact_id: id, tag })))
        : null,
    ]);
  ...
  const displayPhoto = photoPreview ?? photoUrl ?? undefined;
    ...
    <div className="min-h-screen pb-10">
      <div className="sticky top-0 z-10 border-b border-border bg-background/95 backdrop-blur px-4 py-3">
        <div className="mx-auto max-w-lg flex items-center justify-between">
          ...
          <h1 className="font-semibold">Edit contact</h1>
      ...
      <form onSubmit={handleSubmit} noValidate className="mx-auto max-w-lg px-4 py-6 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="relative">
              ...
              <Input value={instagram} onChange={(e) => setInstagram(e.target.value)} placeholder="Instagram" className="pl-8" />
            </div>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-muted-foreground">𝕏</span>
              <Input value={twitter} onChange={(e) => setTwitter(e.target.value)} placeholder="X / Twitter" className="pl-8" />
            </div>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs font-bold text-blue-400">in</span>
              <Input value={linkedin} onChange={(e) => setLinkedin(e.target.value)} placeholder="LinkedIn" className="pl-8" />
            </div>
            <div className="relative">
              <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-purple-400">🎵</span>
              <Input value={tiktok} onChange={(e) => setTiktok(e.target.value)} placeholder="TikTok" className="pl-8" />
            </div>
          </div>
        ...
        <div className="space-y-2">
          ...
          <Input id="how_we_met" value={howWeMet} onChange={(e) => setHowWeMet(e.target.value)} />
        </div>
        ...
        <ChipInput
          ...
          placeholder="Video editing, brand deals..."
          items={offers}
        ...
        <ChipInput
          ...
          placeholder="A developer, investors..."
          items={needs}
        ...
        <ChipInput
          label="Tags"
          placeholder="Fanshawe, collab, investor..."
          items={tags}
        ...
        <Button type="submit" className="w-full h-12" disabled={saving || !name.trim()}>
